...
if TYPE_CHECKING:
    ...
    from .models import (
        Account, Administrator, Company, DestinationForNewUser, DestinationOfAccount,
        License, ProviderAccount, User, UserModeType,
    )
...
class Msp360:
    ...
    def _request(self, method, path, *, params=None, body=None):
        ...
        try:
            ...
            try:
                msg = json.loads(e.response.text).get('Message')
            except Exception:
                raise e
            else:
                raise HTTPStatusError(msg, request=e.request, response=e.response) from None
...
# region LICENSES
    ...
    def grant_license(self, user_id: str, license_id: str):
        ...
        return self.post('Licenses/Grant', body)
    ...
    def release_license(self, user_id: str, license_id: str):
        ...
        '''
        body = {'LicenseID': license_id, 'UserID': user_id}
        return self.post('Licenses/Release', body)
    ...
    def revoke_license(self, user_id: str, license_id: str):
        ...
        '''
        body = {'LicenseID': license_id, 'UserID': user_id}
        return self.post('Licenses/Revoke', body)
...
# region BILLING
    ...
    def get_billing_stats(
        ...
        '''
        Args:
            company_name: If not set results for all companies will be
                returned.
            date: If not set the current month will be returned.
        '''
        if date is not None:
            if date.tzinfo is None:
                date = datetime(*date.timetuple()[:6], tzinfo=timezone.utc)
            date = date.isoformat()

        body = {'CompanyName': company_name, 'Date': date}
        return self.put('Billing', body)
# endregion

...
class UserModeType(IntEnum):
    ...
    Automatic       = 1
    #: Company settings for licensing are applied. Direct changes are
    #:     restricted.
    CompanySettings = 2
...
class LicensingMode(IntEnum):
    '''Company license settings.
    '''
    #: Users have custom license settings.
    Custom      = 0
    #: Users activate paid licenses from the global pool automatically.
    GlobalPool  = 1
    #: Users can activate only limited number of paid licenses from the
    #:     company pool
    CompanyPool = 2
...
class Company(TypedDict):
    ...
    Args:
        Id: Company UUID.
        Name: Company name.
        StorageLimit: Company backup limit. A negative value indicates
            the resource is unconstrained by a quota.
    '''
    Id:              Uuid
    Name:            str
    StorageLimit:    int
    LicenseSettings: LicensingMode
...
class Account(TypedDict):
    ...
    DisplayName:  str
    StorageType:  str
    Destinations: List[DestinationOfAccount]


class ProviderAccount(Account):
    ResellerAccountID: Uuid
